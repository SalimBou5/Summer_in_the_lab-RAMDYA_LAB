...
def sendTarget(real_target, final_target, posMagnet,arrived, path, zaber_dragging,wrong_input):
    
    final_target_old = final_target
    #********************COMPUTER VISION INTERPRETATION*************************
    try:
        print("/////////////////////")
        final_target, zaber_dragging = computerVisionInterpretation(posMagnet,zaber_dragging, wrong_input)
    except Exception:
        print("EXCEPTION 0")
    ...
    if(k%5000==0):
        ...
        if(not dragging):# and not arduino.in_waiting):
            try:    
                ...
                print("Wrong image")
            i=i+1
            real_target, final_target,arrived,path,zaber_dragging =  sendTarget(real_target, final_target, posMagnet, arrived, path, zaber_dragging, wrong_input)
            time.sleep(0.1)
            ...
            if data==b'R':
                ...
                if(data==b'y'):
                    ...
                    if((not escape and not zaber_dragging) or (escape and zaber_dragging) and closeEnough(received_x, real_target[0], 0.2) and closeEnough(received_y, real_target[1], 0.2)):
                        if(not escape):
                            print("Going towards ",real_target)
                        if(escape):
                            print("Drag back")
                    else:
