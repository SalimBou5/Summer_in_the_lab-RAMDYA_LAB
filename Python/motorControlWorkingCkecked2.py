...
#transmitted to arduino to know whether to drag_back (escape==1) or to move(escape ==0)
escape=0
...
'''
    This array encodes the position in pixels of all the goal destinations as well as the node to which they correspond
    --> [px, py, node] with px = x coordinates in pixels
                            py = y coordinates in pixels
                            node = node which corresponds to this position (containing the coordinates in cm)
'''
BALLS_DESTINATION = [
...
'''
    This array contains the 9 nine regions to which the image would be divided to have a more accurate detection.
    --> They correspond to the 9 arenas (each one could be reduced to its 6 corridors)
    --> They are represented as rectangles with [top_left_x, top_left_y, bottom_right_x, bottom_right_y] in pixels
'''
...
THRESHOLD_DETECTION_READY = 20  #Could be adjusted if needed
...
def sendTarget(real_target, final_target, posMagnet,arrived, path, zaber_dragging,wrong_input, stopped):
    ...
    try:
        ...
        print("EXCEPTION COMPUTER VISION INTERPRETATION")
    ...
    if(k%5000==0):
        ...
        if(not dragging and not zaber_dragging):
            try:    
                ...
                print("Wrong image")
            i=i+1
            real_target, final_target,arrived,path,zaber_dragging, stopped =  sendTarget(real_target, final_target, posMagnet, arrived, path, zaber_dragging, wrong_input,stopped)
            time.sleep(0.1)
