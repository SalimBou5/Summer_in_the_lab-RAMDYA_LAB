...
def sendTarget(node,posX,posY,arrived,path, at_rest, zaber_dragging):
    node_old = node

    #********************COMPUTER VISION INTERPRETATION*************************
    try:
        node,escape,path,towards_rest=computerVisionInterpretation(node,posX,posY,path)
    except Exception:
        print("EXCEPTION 0")
        return node,arrived,path, at_rest, zaber_dragging
        ...
        if(not escape):
            ...
            if node_old == node: 
                ...
                if(arrived):  #---> if (posMagnet == posFirstNodeOfTheRemainingPath)
                    ...
                    return node_old,arrived, path,at_rest, zaber_dragging
    ...
    if(k%5000==0):
        ...
        if(not dragging):# and not arduino.in_waiting):
            try:    
                ...
                print("Wrong image")
            i=i+1
            targetNode,arrived,path, at_rest,zaber_dragging =  sendTarget(targetNode,posX,posY,arrived,path, at_rest,zaber_dragging)
            time.sleep(0.1)
